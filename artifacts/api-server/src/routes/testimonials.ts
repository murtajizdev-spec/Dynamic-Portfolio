...
router.post("/", async (req, res) => {
  try {
    const inserted = await db.insert(testimonialsTable).values(req.body).returning();
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const updated = await db.update(testimonialsTable).set(req.body).where(eq(testimonialsTable.id, id)).returning();
