...
router.put("/", async (req, res) => {
  try {
    ...
    const rows = await db.select().from(heroTable).limit(1);
    if (rows.length === 0) {
      const inserted = await db.insert(heroTable).values(data as never).returning();
      return res.json(inserted[0]);
    }
    const updated = await db.update(heroTable).set(data).where(eq(heroTable.id, rows[0].id)).returning();
